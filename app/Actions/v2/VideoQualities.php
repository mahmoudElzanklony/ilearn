...
class VideoQualities
{
    ...
    public static function save_videos_using_commands($video_obj, $compressedFilePath, $name)
    {
        ...
        foreach ($items as $item) {
            ...
            exec("ffmpeg -i $video_obj " . $item['scale'] . " $compressedFilePath$file_name 2>&1", $output, $return_var);

            // Log the output and return status
            Log::info("FFmpeg output for {$item['quality']}: " . implode("\n", $output));
            Log::info("FFmpeg return status for {$item['quality']}: $return_var");
            ...
            if ($return_var === 0) {
                ...
                Log::error("Failed to process quality {$item['quality']} for video $name");
            }
